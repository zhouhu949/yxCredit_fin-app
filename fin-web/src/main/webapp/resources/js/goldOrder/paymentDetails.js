...
$(function (){
    var table;
        ...
        /*    {"data": "state","orderable" : false},*/
            {"data": "repaymentState","orderable" : false,
                "render": function (data, type, row, meta) {
                    if(data==0) return "<span style='text-decoration: none;color:#307ecc;'> 扣款中</span>";
                    if(data==1) return "<span style='text-decoration: none;color:#307ecc;'> 已扣款</span>";
                    if(data==2) return "<span style='text-decoration: none;color:red;'> 扣款失败</span>";
                    if(data==3) return "<span style='text-decoration: none;color:#307ecc;'> 未扣款</span>";
                }
            },
            {"data": "state","orderable" : false,
                "render": function (data, type, row, meta) {
    ...
    if(g_userManage.tableOrder){
        ...
        g_userManage.tableOrder = $('#order_list').dataTable($.extend({
            ...
            "ajax" : function(data, callback, settings) {//ajax配置为function,手动调用异步查询
                ...
                Comm.ajaxPost('goldPayment/orderList', JSON.stringify(queryFilter), function (result) {
                    //封装返回数据
                    debugger
                    var returnData = {};
            ...
            "createdRow": function ( row, data, index,settings,json ) {
                ...
                var loanBtn=$('<a  class="tabel_btn_style" style="text-decoration: none;color: #307ecc;" onclick="orderLoan(\''+data.id+'\')"> 扣款明细</a>');
                if((data.repaymentState=="2")){
                    return $("td", row).eq(13).append(debitBtn).append(loanBtn);
                }else {
                    return $("td", row).eq(13).append(loanBtn);
                }
            },
...
function orderLoan(orderId){
    ...
    g_CustomerOrderManage.tableCusOrder = $('#withdrawal_list').dataTable($.extend({
        ...
        "dom": 'rt<"bottom"i><"bottom"flp><"clear">',
        "ajax" : function(data, callback, settings) {//ajax配置为function,手动调用异步查询
            var queryFilter = g_CustomerOrderManage.getQueryCondition(orderId,data);
            debugger
            Comm.ajaxPost('goldPayment/transactionDetailsList', JSON.stringify(queryFilter), function (result) {
                //封装返回数据
                var returnData = {};
                var resData = result.data.list;
                var resPage = result.data;
...
function  orderDebit(orderId) {
    Comm.ajaxPost('goldPayment/costDebit','orderId='+orderId,function(data){
