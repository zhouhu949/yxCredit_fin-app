var g_userManage = {
    ...
    getQueryCondition : function(data) {
        ...
        param.deliveryState='0'; //发货状态
        if (g_userManage.fuzzySearch) {
            param.customerName = $("input[name='customerName']").val();
            param.deliveryState=$('#isLock').val(); //获取订单状态
    ...
    if(g_userManage.tableOrder){
        ...
        g_userManage.tableOrder = $('#order_list').dataTable($.extend({
            ...
            "createdRow": function ( row, data, index,settings,json ) {
                ...
                var btnUpdate=$('<a class="tabel_btn_style" onclick="updateOd(\''+data.id+'\')"> 确认收货</a>');
                ...
                if (data.delivery_state=="2"){
                $('td', row).eq(12).append(btnDel).append(btnUpdate);
                }else {
...
function  addLogistics(orderId) {
    layer.open({
        ...
        yes : function(index, layero) {
            ...
            if(param.purchasePrice==""){
                layer.msg("采购价格格式有误！",{time:2000});
                return;
            }
            ...
            if(param.companyId=="-1"){
                layer.msg("请选择物流公司！",{time:2000});
                return;
            }

            if(param.billNo==""){
                layer.msg("请填写物流单号！",{time:2000});
                return;
            }
...
//确认收货
function  updateOd(orderId) {
    Comm.ajaxPost('goldCustomerAudit/updateOrderDelivery', orderId, function (result) {
