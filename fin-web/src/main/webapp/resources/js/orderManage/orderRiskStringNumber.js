/**
 * 商品串号码管理的js
 * @type {{tableOrder: null, currentItem: null, fuzzySearch: boolean, getQueryCondition: g_userManage.getQueryCondition}}
 */
var g_userManage = {
    tableOrder : null,
    currentItem : null,
    ...
    getQueryCondition : function(data) {
        ...
        if (g_userManage.fuzzySearch) {
            param.merchandiseCode=$.trim($("#merchandiseCode").val());
    ...
    if(g_userManage.tableOrder){
        ...
        g_userManage.tableOrder = $('#order_list').dataTable($.extend({
            ...
            "ajax" : function(data, callback, settings) {//ajax配置为function,手动调用异步查询
                ...
                Comm.ajaxPost('orderRiskStringNumber/getSubmitList', JSON.stringify(queryFilter), function (result) {
                    debugger
                    var returnData = {};
            ...
            "columns": [
                ...
                {"data": "merchandiseCode","orderable" : false},//商品串号码
                {"data": "null", "orderable": false, "defaultContent":""}
            ],
            "createdRow": function ( row, data, index,settings,json ) {
               //设置商品串号码列的颜色
                $('td',row).eq(14).css({"color":"red"});
