...
$(function (){
    g_userManage.tableUser = $('#afterLoanTable').dataTable($.extend({
        ...
        "ajax" : function(data, callback, settings) {//ajax配置为function,手动调用异步查询
            ...
            Comm.ajaxPost('afterLoan/listReminders', JSON.stringify(queryFilter), function (result) {
                console.log(result);
                //封装返回数据
                var returnData = {};
        ...
        "columns": [
            ...
            {"data": "external_auditor_date","orderable" : false,
                "render": function (data, type, row, meta) {
            ...
            {"data": "external_auditor_state","orderable" : false,
                "render": function (data, type, row, meta) {
                    if(data.toString()=="1"){
                        return "进行中";
                    }else {
        ...
        "createdRow": function(row, data, index, settings, json) {
            ...
            var collectionbtn = $('<a href="##" style="text-decoration:none;color: #307ecc;" class="tabel_btn_style" onclick="collection(\''+data.customerId+'\')"> 委外处理</a>');
            //$("td", row).eq(6).append(btn).append(planbtn).append(collectionbtn);
            $("td", row).eq(6).append(btn).append(collectionbtn);
        },
...
function collection(customerId) {
    ...
    var param ={
        ...
        entrust_state:"1"}
    Comm.ajaxPost('afterLoan/updateEntrusted',JSON.stringify(param),function(data){
        layer.msg(data.msg,{time:2000},function () {
            g_userManage.tableUser.ajax.reload();
