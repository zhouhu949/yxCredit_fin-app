...
$().ready(function(){
    ...
    var personList,deviceInfo,linkmanList,jobList,imageList,host,ruleResult,servicePackageOrderList,answerList,answerScore,salesman;
    var customer;
    var paramMap = {};
    ...
    if(servicePackageOrderList){
        ...
        for (var i=0;i<servicePackageOrderList.length;i++){
            ...
            if(servicePackageOrder.type=='1'){
                if (servicePackageQZ==""){
                    servicePackageQZ='<br /><table style="width: 100%"><tr><td colspan="4" style="text-align: left">前置服务包</td></tr>' +
                        '<tr><td style="width: 25%">还款包类型</td><td style="width: 25%">还款包名称</td><td style="width: 25%">价格</td><td style="width: 25%">几期后允许提前还款</td></tr>';
                    servicePackageQZ+='<tr><td>'+servicePackageOrder.packageTypeName+'</td><td>'+servicePackageOrder.packageName+'</td><td>'+servicePackageOrder.amountCollection+'</td><td>'+servicePackageOrder.month+'</td></tr>';
                }else {
                ...
                if (servicePackageTQ==""){
                    ...
                    servicePackageTQ+='<tr><td>'+servicePackageOrder.packageTypeName+'</td><td>'+servicePackageOrder.packageName+'</td><td>'+servicePackageOrder.amountCollection+'</td><td>'+servicePackageOrder.month+'</td><td>'+(servicePackageOrder.periodCollection ? servicePackageOrder.periodCollection : '--')+'</td></tr>';
                }else {
                    servicePackageTQ+='<tr><td>'+servicePackageOrder.packageTypeName+'</td><td>'+servicePackageOrder.packageName+'</td><td>'+servicePackageOrder.amountCollection+'</td><td>'+servicePackageOrder.month+'</td><td>'+(servicePackageOrder.periodCollection ? servicePackageOrder.periodCollection : '--')+'</td></tr>';
                }
    ...
    if(salesman){
        ...
        $('#tdRepayDate').html(getTime1($('#tdRepayDate').html()));//到期时间
        //四舍五入分期金额
            ...
            if(ruleResult[i].engineId=='286L'){
                ...
                ruleResultHtml=ruleResultHtml+
                    '<tr>' +
                        ...
                        '<td>' +
                            ...
                            // '<a class="tabel_btn_style" href="'+_ctx+'/customer/get91ZX?customerId='+customer.id+'" target="_blank"> 91征信报告 </a>' +
                        '</td>'+
                    '</tr>';
                ...
                if(customer){
                    ...
                    //if(customer.callRecordUrl&&customer.callRecordUrl!=''){
                        ...
                        HDHtml='<a class="tabel_btn_style" href="'+_ctx+'/customer/getTelephoneRecord?customerId='+customer.id+'&orderId='+orderId+'" target="_blank"> 话单下载 </a>';
                ...
                ruleResultHtml=ruleResultHtml+'<tr><td width="10%">规则名称：</td><td width="23%">运营商</td><td width="10%">审核结果：</td><td colspan="2">'+ruleResult[i].state+'</td><td>'+HDHtml+'</td></tr>';
            }
