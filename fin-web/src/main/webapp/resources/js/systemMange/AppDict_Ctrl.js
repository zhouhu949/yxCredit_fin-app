...
function queryList(id){
    g_userManage.tableUser = $('#Res_list').dataTable($.extend({
        ...
        "ajax" : function(data, callback, settings) {
            ...
            Comm.ajaxPost("appDict/listDict", JSON.stringify(queryFilter), function (result) {
                console.log(result);
                debugger
                var returnData = {};
        ...
        "columns" :[
            ...
            },
            {
                ...
                "orderable" : false,
                "render" : function(data, type, row, meta) {
                    return json2TimeStamp(data.alterTime);
                }
            },
...
function openDict(e){
    ...
    titleName = "编辑";
    Comm.ajaxGet(
        "appDict/detail?id="+checkId,
        "",
        function(data){
            var dict = data.data;
            $('input[name="name_code"]').val(dict.code);
            $('input[name="name_dict"]').val(dict.name);
            $('textarea[name="remark"]').val(dict.description);
            $("#parentId").val(dict.dictId);
            editDict(checkId);
        }
    );
}
...
function editDict(checkId){
    ...
    layerIndex = layer.open({
        ...
        yes:function(index, layero){
            ...
            if(name==''){
                layer.msg("字典名称不可为空！",{time:1000});return
            }
            if(code==''){
                layer.msg("字典Code不可为空！",{time:1000});return
            }
            Comm.ajaxPost(
                'appDict/edit',JSON.stringify(dict),
                function(data){
                    if(data.code==1){
...
function deleteById(e){
    ...
    var param = {};
    param.id = checkId;
    Comm.ajaxPost('appDict/delete',JSON.stringify(param),
        function(data){
...
//删除字典明细
function deleteDetailById(e){
    var target = e.target || window.event.target;
    var checkId = $(target).parents("tr").children().eq(1).children("input").val();//获取点击的id
    var param = {};
    param.id = checkId;
    Comm.ajaxPost('appDict/deleteDetail',JSON.stringify(param),
        function(data){
            layer.msg(data.msg,{time:2000});
            g_userManage.tableUser.draw();//刷新数据
            g_userDetileManage.tableUser.draw();//刷新数据
...
function addDict(){
    ...
    layerIndex = layer.open({
        ...
        yes:function(index, layero){
            ...
            Comm.ajaxPost(
                'appDict/add',JSON.stringify(param),
                function(data){
                    if(data.code==1){
                        layer.msg(data.msg,{time:1000});
                    }
...
function queryListDetile(menuId){
    g_userDetileManage.tableUser = $('#Res_list_detail').dataTable($.extend({
        ...
        "ajax" : function(data, callback, settings) {
            ...
            Comm.ajaxPost("appDict/listDictDetail", JSON.stringify(queryFilter), function (result) {
                debugger
                var returnData = {};
        ...
        "createdRow": function ( row, data, index,settings,json ) {
            var btnAdd = $('<a href="##" style="text-decoration:none;color: #307ecc;" class="" onclick="deleteDetailById(event)">删除&nbsp;</a>');
...
function addDel(e){
    ...
    layerIndex = layer.open({
        ...
        yes:function(index, layero){
            ...
            Comm.ajaxPost(
                'appDict/addDetail',JSON.stringify(dict),
                function(data){
                    if(data.code==1){
                        layer.msg(data.msg,{time:1000});
                    }
...
function editDictDel(checkId,parentId){
    ...
    layerIndex = layer.open({
        ...
        yes:function(index, layero){
            ...
            Comm.ajaxPost(
                'appDict/editDetail',JSON.stringify(dict),
                function(data){
                    if(data.code==1){
                        layer.msg(data.msg,{time:1000});
                    }
