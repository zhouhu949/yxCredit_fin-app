$().ready(function(){
    ...
    var personList,deviceInfo,linkmanList,jobList,imageList,host,servicePackageOrderList,answerList,answerScore;
    var customer;
    var paramMap = {};
    ...
    if(customer){
        //if(customer.callRecordUrl&&customer.callRecordUrl!=''){
            var HDHtml='<a class="tabel_btn_style" href="'+_ctx+'/customer/getTelephoneRecord?customerId='+customer.id+'" target="_blank"> 话单下载 </a>';
        ...
        for(var i=0;i<ruleResult.length;i++){
            ...
            if(ruleResult[i].engineId=='286L'){
                ...
                ruleResultHtml=ruleResultHtml+
                    '<tr>' +
                        ...
                        '<td>' +
                            ...
                            '<a class="tabel_btn_style" href="'+_ctx+'/customer/getTonDunZX?customerId='+customer.id+'&orderId='+orderId+'" target="_blank"> 同盾报告 </a>' +
                        '</td>'+
                    '</tr>';
                ...
                if(customer){
                    var  HDHtml="";
                    //if(customer.callRecordUrl&&customer.callRecordUrl!=''){
                        //HDHtml='<a class="tabel_btn_style" href="'+customer.callRecordUrl+'" target="_blank"> 话单下载 </a>';
...
function clickNextButton(){
    layerIndex = layer.open({
        ...
        yes:function(index, layero){
            ...
            var relId=$("#relId").val();
            Comm.ajaxPost('suretyCheckController/approved',JSON.stringify({id:orderId,approveSuggestion:approveSuggestion,customerId:customerId,taskNodeId:taskNodeId,relId:relId,state:orderState}) , function (result) {
                layer.msg(result.msg,{time:2000},function(){
                    // layer.closeAll();
                    var index = parent.layer.getFrameIndex(window.name);
                    parent.g_userManage.tableUser.ajax.reload();
...
function reasonClick(){
    ...
    layerIndex = layer.open({
        ...
        yes:function(index, layero){
            ...
            var customerId = $("#customerId").val();
            Comm.ajaxPost('suretyCheckController/refused',JSON.stringify({id:orderId,approveSuggestion:approveSuggestion,customerId:customerId,userId:userId,relId:relId,state:orderState}) , function (result) {
                layer.msg(result.msg,{time:2000},function(){
                    // layer.closeAll();
                    var index = parent.layer.getFrameIndex(window.name);
                    parent.g_userManage.tableUser.ajax.reload();
